...
namespace HyperledgerTest
{
    public class UserManager
    {
        ...
        public UserManager()
        {
            ...
            var peer = fabric_client.NewPeer("p1", $"grpc://{Program.ServerIP}:7051");
            channel.AddPeer(peer);

            var ordered = fabric_client.NewOrderer("o1", $"grpc://{Program.ServerIP}:7050");
            channel.AddOrderer(ordered);

            channel.Initialize();

            fabric_ca_client = new Hyperledger.Fabric_CA.SDK.HFCAClient("", $"http://{Program.ServerIP}:7054", new Hyperledger.Fabric.SDK.Helper.Properties());
